...
def initialize_app():
    ...
    # 設定値をログ出力
    logger.info("========================================")
    logger.info("LeverAPI 起動設定:")
    logger.info(f"  Host: {lever_config.get('bind', '0.0.0.0')}")
    logger.info(f"  Port: {lever_config.get('port', 5001)}")
    logger.info(f"  Discovery Port: {discovery_port}")
    logger.info(f"  Broadcast Address: {broadcast_address}")
    logger.info(f"  Poll Interval: {lever_config.get('pollInterval', 1000)}ms")
    logger.info(f"  Update Interval: {UPDATE_INTERVAL}秒 ({1/UPDATE_INTERVAL:.1f}Hz)")
    if local_ips:
        logger.info("  System IP Addresses:")
        for ip in local_ips:
            logger.info(f"    - {ip}")
    else:
        logger.info("  System IP Addresses: (none detected)")
    logger.info("========================================")
